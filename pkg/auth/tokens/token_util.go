...
func GetKubeConfigToken(userName, authProvider, responseType string, userMGR user.Manager) (*v3.Token, string, error) {
	...
	token, tokenVal, err := userMGR.GetKubeconfigToken(clusterID, name, "Kubeconfig token", "kubeconfig", userName, authProvider)
	if err != nil {
		return nil, "", err
	}
